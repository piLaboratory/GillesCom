...
metacommunity of of expected \footnote{This function generates $S$ values of abundances from a logseries distribution. Therefore the total number of individuals in the simulated metacommunity varies among runs of \code{ls.m}, but has an expected value of $J$} size $J$ and $S$ species  with a given Fisher's alpha and a per species migration rate of $m$:
...
in \code{history}. This is because the lines in \code{history} is recorded whenever
the clock reaches a designated interval (by default, every unit of time), while \code{abundance}
gives the abundance at precisely the instant given by \code{time}.
