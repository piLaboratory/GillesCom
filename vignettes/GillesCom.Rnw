...
So far all rates were constant for each species. Currently it is posible to make birth rates vary over time. 
...
birth <- exp(cumsum(rnorm(intervals, sd=sd)))
birth.df <- data.frame(time = seq(0, Time, length=intervals), birth)
plot(birth.df, type='l')
@

Then, we generate a community under the effect of this sotchastic birth rate multiplier as:
<<>>=
stochcom = Init_Community(50, stochastic=birth.df)
