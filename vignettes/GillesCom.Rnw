...
<<Init>>=
com = Init_Community(abundance=rep(0, N), interaction=random, b=1, m = migration, save.int=1)
@

Many arguments 
of this function are expanded to vectors of the appropriate length, if
necessary. This is the case for $m$ (migration rate), $b$ (birth rate)  and $d0$ (basal death rate),
for example. See the help of \code{Init\_Community} for the full list of parameters. 
...
Finally, notice that \code{bdm} is incremental. A new call will run simulations starting from the state of the community
at the end of the last simulation, until a new community is set by \code{Init\_Community}.
...
The following lines of code generate a data frame with two columns, the first representing the time and the second representing a common multiplier for the carrying capacities of all species due to environmental stochasticity that performs a random walk. 
With the parameters below, a typical run will be constrained in the 0.25 - 4 interval.

<<stochasticity>>=
K.df = stoch_environment(time = 30, sd = 0.01)
plot(K.df, type='l')
@
...
The last value of the stochastic multiplier is kept for
runs beyond the maximum time in the environmental stochasticity data frame.

\subsection{Species sensitivities to environmental stochasticity}
...
the second will only suffer half the effect of the environment (\code{str = 0.5}), so when the environmental 
...
bdm(stochcom2, time=400)
matplot(stochcom2$trajectories[[1]], type='l')
legend("topleft", legend=c("Fully affected", "Half affected", "Not affected", "Reverse affected"), 
